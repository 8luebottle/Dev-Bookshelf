...
<details><summary>이름 짓기</summary>
프로그래밍에서 이름은 모든것.

- 이름을 지을때: 표현하고자하는 것이 최대한 명확하게 드러나도록.
  - 좋은 이름은 코드의 이해도를 높혀준다.
  - 언어의 문화를 존중할 것.
  - 일관성 유지하기.
    - 팀의 모든 사람이 각 단어의 뜻을 알고 일관성 있게 사용해야 함.

### 이름 바꾸기는 더 어렵다

- 문제 발견 당시 고치기.
  - 의도를 제대로 표현하지 못한 이름인가?
  - 오해를 부를만한 이름인가?
  - 헷갈리는 이름인가?
- 지나치게 일반적인 이름이라면 실제로 하는 일이 반영되도록 이름 바꾸기.
- 시스템에서 사용하는 이름이 도메인 사용자들과 다르다면 그 이유는?
...
> Before the Project 项目启动之前

프로젝트의 닻을 올리기 전에 무엇을 확실히 해두어야 할까?

<details><summary>요구 사항의 구렁텅이</summary>

`완성이라는 것은 더 이상 더할 것이 없을 때가 아니라, 더 이상 뺄 것이 없을 때 달성되는 것이다. - Antoine de St. Exupery`

`자신이 뭘 원하는지 정확히 아는 사람은 아무도 없다.`

엉망진창인 세상에서 정확한 명세란 것은 없다.

신입 개발자가 하는 실수는 요청 사항을 받았을때 바로 구현 시작하기. 문제는 최초의 요청사항은 궁극적인 요구사항이 아니라는 것. 요청사항은 계속 변한다. 프로그래머는 요청한 사람이 자신이 무엇을 원하는지 깨닫도록 도와줘야 한다. 질문을 통해...

- 구현할 계획을 기준으로 질문하기.
  - 예) `5만원 이상인 모든 주문은 배송비가 무료.`
    - 어떤 종류의 배송인가? 당일배송, 일반 배송?
    - 국제 배송은?
    - 5만원은 배송비도 포함인가? 세금은? ...등등

### 요구 사항은 과정이다

`요구 사항은 피드백을 반복하며 알개 된다.`

- 피드백을 받은 의뢰인은 자신의 생각을 다듬어 나가게 된다.
- mockup 이나 프로토타입을 통해 의뢰인이 직접 사용해 볼 수 있도록 하기.
  - 프로토타입, 예광탄을 사용하여 빠른 피트백 받기.
- 프로젝트 전체를 요구 사항 수집 과정으로 보기.
  - 이를 짧은 주기로 계속 반복하기.
...
- 요구 사항인지 정책인지 비슷한 둘을 구분해야 한다.

### 요구 사항 문서화

- 요구 사항을 문서로 정리.
  - 문서는 구현 과정에서 안내 역할을 하는 이정표.
- 요구 사항 문서는 의뢰인을 위한 것이 아니다.
  - 이는 개발자를 위한 것.
  - 의뢰인은 모호한 풀고 싶어하는 문제를 던져주면 프로그래머는 세부사항을 제시한다.
  - 요구 사항 문서는 계획을 위한 것.
    - 요구 사항의 설명을 붙이되 자세하게 하지는 않는다. 그로인해 그 기능을 하는 개발자와 요구자 간에 피드백 과정이 활발히 이루어진다.

### 지나치게 자세한 명세

- 좋은 요구 사항은 추상적. 모호하게 작성하라는 의미는 아님.
  - 명세에 의미론적 불변식 포함하기.
  - 구체적인 작업 방식이나 현재의 작업방식은 정책으로.

### 용어 사전 관리하기

- 프로젝트 용어 사전을 만들고 관리하기.
- 일관성을 위해 프로젝트에 참여하는 모든 사람이 동일한 용어 사전을 사용하도록하기.
  - 사용자와 개발자가 동일한 것을 다른 이름으로 부른다면 프로젝트는 성공하기 힘듬.
...
> Pragmatic Projects 务实的项目

<details><summary>실용주의 팀</summary>

실용주의 팀 사이즈

- 10-12 명 이하.
  - 서로가 서로를 잘 알고, 신뢰하며 의존할 수 있는 관계.
  - 작고 안정적인 팀 유지하기.

### 깨진 창문을 없애라

- 품질은 팀에 문제.
  - 팀 구성원들이 품질에 무심하지 않도록 하기.
- 팀 전체가 깨진 창문을 용납하지 않도록 하기.

### 삶은 개구리

- 팀은 개인보다 삶은 개구리가 되기 쉬움.
  - 누군가가 문제를 해결하겠지.. X
  - 모든 팀 구성원이 변화에 민감하도록.
    - 범위 확장
    - 추가 기능
...
- 구형 시스템 유지 보수
  - 유지보수 작업을 구석에 쳐박아놓지 않가.
- 프로세스 회고와 개선
- 새로운 기술 탐험
  - 새로운 기술, 프레임워크, 라이브러리
    - 새로운 것으로 프로토타입을 만들어 보기.
    - 신중하게 조사하기.
- 학습 및 갈고 닦기
  - 배우며 역량 키우기. 개인이 아닌 팀원이 함께.

### 반복하지 말라

- `좋은 의사소통`은 반복이라는 문제를 피할 수 있게 해준다.
  - 좋은 의사소통: 즉각적이고 매끄러움.
  - 팀 동료에게 질문하고 즉각적으로 답을 받을 수 있는 환경이 구성되어 있는가?
  - 서로 관심 유지하기.
...
- 예광탄을 만들 수 있는 팀을 구성하기.
- 팀은 개인들로 이루어져 있으며 자신만의 방식대로 빛나게 해주기.
- 성공적이 팀 사례를 찾아보자.

  - 그 팀을 성공적으로 만든 요소는?

- 시스템에서 끝에서 끝까지 걸쳐있는 단일 기능 개발하기.
  - 처음에는 작고 제한적일 수 있음.
  - 프로젝트가 어떤 맥락에서 사용되는지 알 수 있게됨.
  - 이를 위해서 기술 팀 전체가 함께 일하는것이 익숙하고 편해야 함.
    - 팀: 프론트엔드, UI/UX, 서버, DBA, QA 등

### 자동화

- 자동화를 통해 팀이 하는 모든 일에 일관성과 정확성을 부여할 수 있다.
  - 코드 스타일, 빌드, 배포, 테스트 자동화.
    - 자동화되어 있지 않다면 만들어 적용시키기.
