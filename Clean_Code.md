...
|-------|:----------------:|:-----:|:--------:|
|[01](#1)|깨끗한 코드          |001-020|07.27.2020|
...
<details><summary>장인 정신</summary>

장인 정신을 익히는 과정은 두 단계 : 이론과 실전.

1. 장인에게 필요한 윈칙, 패턴, 기업, 경험이라는 지식을 습득해야 한다.
...
<details><summary>코드가 존재 하리라</summary>

*'코드를 자동으로 생성하는 시대가 다가오고 있다. 그렇기에 코드의 종말이 왔다'* 는 주장은 틀렸다.

코드는 요구사항을 상세히 표현하는 수단이기에 사라질 수 없다.

모호한 요구사항을 토대로 인간이 원하는 그대로 완벽히 실행해 내도록 만드는 기계는 존재할 수가 없다.  
- "소금 몇 스푼 넣을까요 ?"
- "적당히"
- "안 짠 만큼만"  
  <img width="140" alt="Conf" src="https://user-images.githubusercontent.com/48475824/131238902-6844263c-2224-47e1-8f3c-07dc3a932a21.png">

</details>

<details><summary>나쁜 코드</summary>

우리가 고행을 맞딱들이는 이유는 나쁜 코드 때문. 나쁜 코드는 우리의 발목을 잡는다.

**[나쁜 코드를 짜게 되는 이유]**

1. 제대로 짤 시간이 시간이 없음
2. 서둘러서 코드를 짜야 함
3. 코드를 다듬느라 쓰는 시간때문에 상사에게 한 소리 들을까봐
4. 지겨워서 빨리 넘기고 싶어서
5. 쌓인 그 다음 업무

나쁜 코드를 짜면서 '미래의 나'에게 쓰레기를  손봐달라는 부탁을 한다.  
'어쨋거나 돌아가기만 하면 된거지... 미래의 나야 나중에 이 고약한 쓰레기를 나중에 치워다오'
- 하지만.... 그 나중은 오지 않았다고 한다.  
   Leblanc's Law States : "Later == never"

</details>

<details><summary>나쁜 코드로 치루는 대가</summary>

나쁜 코드는 개발 속도를 크게 저하 시킨다.

초반에는 스피드가 나지만 장기적으로 봤을때 시간이 더뎌짐
...
생산성이 떨어지는 것을 발견한 관리층은 이를 해결하고자 여러 방법을 시도.

- 추가 인력 투입

    하지만 추가 된 인력은 쓰레기 코드와 설계에 대한 이해도가 부족

    —> 결론적으로 나쁜코드 더 양산

</details>

<details><summary>원대한 재설계의 꿈</summary>

프로그래머는 제대로 쓰레기 산을 정리하고자 관리층에게 재 설계 요구.

관리층에서는 '재설계'를 위한 새 팀을 구성하게 된다.

새 팀의 업무는

- 기존 시스템 개선한 새 시스템 내놓기
- 위의 업무 진행되는 동안 기존 시스템에서 변경된 점을 적용하기

하지만... 이 업무를 완성하기 위해 10년이 걸리는 경우도 있다.

</details>

<details><summary>태도</summary>

엉망인 코드에 대해 온갖 변명을 늘여 놓지 말것.

- 관리자가 준 시간이 한정적이다.
- 설계가 뒤집혀 요구 사항이 변했다.
- 일정이 촉박해 관리할 시간이 없다.

프로그래머 자신의 태도를 바로 잡아야 한다. 잘못이 프로그래머에게도 있는 이유.

- 관리자와 마케팅은 약속과 공약을 내걸며 ***프로그래머*** 에게 정보를 구한다.
- 사용자는 요구사항을 내놓으며 ***프로그래머*** 에게 도움을 청한다.

우리는 프로젝트를 계획하는 과정에 깊숙히 관여한다. 그러므로 프로젝트 실패는 우리에게도 커다란 책임이 있다. 특히 나쁜 코드가 초래하는 실패에는 더더욱 책임이 크다.

일정과 요구사항을 강력히 밀어붙이는 것은 관리자의 책임이고, 우리의 책임은 좋은 코드를 사수시키는 것.

나쁜 코드의 위험을 이해하지 못하는 관리자 말을 그대로 따르는 행동은 전문가답지 못하다.

</details>

<details><summary>원초적 난제</summary>

프로그래머의 난제.

- 코드의 질 인가? 속도인가?

기한을 맞추는 유일한 방법은, 언제나 코드를 최대한 깨끗하게 유지하는 습관. 나쁜 코드를 양산하면 기한을 맞추지 못한다. 오히려 엉망진창인 상태로 인해 속도가 곧바로 늦어지고, 결국 기한을 놓친다.

</details>

<details><summary>깨끗한 코드라는 예술?</summary>

깨끗한 코드를 작성하는 프로그래머는 빈 캔퍼스를 우아한 작품으로 바꿔가는 화가와 같다.

깨끗한 코드를 작성하려면 자잘한 기법들을 적용하는 절제와 규율이 필요하다.

이를 *'코드 감각'* 이라고 부른다.

- ‘코드 감각'이 있으면 좋은 코드와 나쁜 코드를 구분한다.
- ‘코드 감각’이 있는 프로그래머는 나쁜 모듈을 보면 좋은 모듈로 개선할 방안을 떠올린다.
- 최고 방안을 선택한 후 여기서 거기까지 이동하는 경로를 계획한다.

</details>

<details><summary>깨끗한 코드란?</summary>

- 깨끗한 코드는 **한 가지만 하는 코드**이다.

    `나쁜 코드`는 너무 많은 일을 하려 애쓰다가 의도가 뒤섞이고 목적이 흐려진다. 

    `깨끗한 코드`는 한 가지에 ‘집중’한다. 각 함수와 클래스와 모듈은 주변 상황에 현혹되거나 오염되지 않은 채 한길만 걷는다.

- 깨끗한 코드는 **세세한 사항까지 꼼꼼하게 처리하는 코드**다.
    - 메모리 누수
    - 경쟁 상태 race condition
    - 일관성 없는 명명법
- 깨끗한 코드는 **가독성이 좋은 코드**이다.
    - 깨끗한 코드는 해결할 문제의 긴장을 명확히 드러내야 한다.
    - 코드는 추측이 아니라 사실에 기반해야 한다.
    - 반드시 필요한 내용만 담아야 한다.
    - 코드를 독해하느라 머리를 쥐어짤 필요가 없어야 한다. 읽으면서 짐작한 대로 돌아가는 코드여야 한다.
- 깨끗한 코드는 **고치기 좋은 코드**이다.
    - 읽기 쉬운 코드와 고치기 쉬운 코드는 엄연히 다르다.
    - 테스트 주도 개발(Test Driven Development)이라는 분야는 우리 업계에 심오한 영향을 미치면서 오늘날 가장 근본적인 원칙 중 하나가 되었다. 테스트 케이스가 없는 코드는 깨끗한 코드가 아니다. 아무리 코드가 우아해도, 아무리 가독성이 높아도, 테스트 케이스가 없으면 깨끗하지 않다.
- 깨끗한 코드란 **중복이 없는 코드**이다.
    - 같은 작업을 여러 차례 반복한다면 코드가 아이디어를 제대로 표현하지 못한다는 증거.

</details>

<details><summary>보이스카우트 규칙</summary>
